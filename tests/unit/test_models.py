...
class WriterTests(TestCase):
    ...
    def test_writer_has_name(self):
        writer = Writer.objects.create(name="Saudia")
        self.assertEqual(writer.name, "Saudia")
...
class ReviewerTests(TestCase):
    ...
    def setUp(self):
        self.writer = Writer.objects.create(name="Saudia")
        self.session = Session.objects.create(writer=self.writer)

    def test_reviewer_has_name(self):
        reviewer = Reviewer.objects.create(name="Yusuf", session=self.session)
        self.assertEqual(reviewer.name, "Yusuf")

    def test_duplicate_reviewer_name_in_same_session_is_rejected(self):
        Reviewer.objects.create(name="Yusuf", session=self.session)
    ...
    def test_same_reviewer_name_allowed_in_different_sessions(self):
        ...
        Reviewer.objects.create(name="Yusuf", session=self.session)
        Reviewer.objects.create(name="Yusuf", session=other_session)
        self.assertEqual(Reviewer.objects.filter(name="Yusuf").count(), 2)
